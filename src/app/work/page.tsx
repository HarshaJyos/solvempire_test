    ...
    <div className="flex flex-col min-h-screen bg-[var(--surface-canvas)] text-[var(--text-body)] selection:bg-[#FACC15] selection:text-[#181A1D] font-sans">
      ...
      <main id="main-content" className="flex-1 w-full pt-36 pb-28">
        <div className="indisea-wrap space-y-16">
          ...
          {/* Case Studies Grid (Indisea Card Style - Compact & Sharp) */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            {caseStudies.map((study, idx) => (
              <article
                key={study.slug}
                className="group relative rounded-none bg-[var(--surface-card)] border border-[var(--border-hairline)] hover:border-slate-400 overflow-hidden shadow-xs transition-all duration-300 flex flex-col justify-between"
              >
                {/* Visual Thumbnail */}
                <div className="relative aspect-[16/7.5] w-full bg-slate-900 overflow-hidden border-b border-[var(--border-hairline)]">
                  ...
                  <div className="absolute top-3 left-3 z-10">
                    <span className="px-2.5 py-0.5 rounded-none bg-[var(--surface-card)]/90 backdrop-blur-md border border-[var(--border-hairline)] font-mono text-[10px] font-bold text-[var(--text-heading)] uppercase shadow-xs">
                      {study.category}
                    </span>
                  </div>

                  <div className="absolute top-3 right-3 z-10">
                    <span className="px-2.5 py-0.5 rounded-none bg-emerald-600 text-white font-mono text-[10px] font-bold uppercase shadow-xs">
                      {study.status}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between gap-4">
                  <div className="space-y-2">
                    ...
                    <h2 className="font-display text-lg sm:text-xl font-extrabold tracking-tight text-[var(--text-heading)] leading-snug group-hover:text-[#1F56C6] transition-colors uppercase">
                    ...
                    {study.metrics && study.metrics.length > 0 && (
                      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[var(--border-hairline)]">
                        {study.metrics.slice(0, 2).map((m) => (
                          <div
                            key={m.label}
                            className="p-2.5 bg-[var(--surface-canvas)] rounded-none border border-[var(--border-hairline)]"
                          >
                            <span className="block font-mono text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wide truncate">
                              {m.label}
                            </span>
                            <span className="font-display text-sm font-bold text-[var(--text-heading)] mt-0.5 block truncate uppercase">
                              {m.value}
                            </span>
                          </div>
                        ))}
                  ...
                  <div className="pt-3 border-t border-[var(--border-hairline)] flex flex-wrap items-center justify-between gap-2 text-xs">
                    <div className="flex flex-wrap gap-1">
                      {study.disciplines.slice(0, 3).map((d) => (
                        <span
                          key={d}
                          className="px-2 py-0.5 rounded-none font-mono text-[10px] font-medium bg-[var(--surface-canvas)] text-[var(--text-muted)] uppercase"
                        >
                          {d}
                        </span>
                      ))}
                    ...
                    <span className="inline-flex items-center gap-1 font-display font-bold text-xs text-[#1F56C6] group-hover:translate-x-1 transition-transform uppercase tracking-wider">
