...
export function JournalRelated({
  ...
  return (
    <section className={cn("my-12 pt-8 border-t border-white/[0.08] space-y-6", className)}>
      <div className="flex items-center justify-between">
        ...
        <Link
          href="/journal"
          className="hidden sm:inline-flex items-center gap-1 text-xs font-semibold text-brand-light hover:text-white transition-colors"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        {displayPosts.map((post) => (
          <article
            key={post.id}
            className="group relative flex flex-col justify-between p-5 sm:p-6 rounded-2xl bg-[#161F2E]/70 border border-white/10 hover:border-brand/40 transition-all hover:bg-[#161F2E] shadow-md hover:-translate-y-0.5"
          >
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 rounded-full bg-brand/10 text-brand-light text-[11px] font-semibold border border-brand/20">
                  {post.category}
                </span>
              ...
              <h4 className="font-heading font-bold text-base sm:text-lg text-[#F8FAFC] group-hover:text-brand-light transition-colors line-clamp-2 leading-snug">
              ...
              <p className="font-body text-xs sm:text-sm text-slate-400 line-clamp-2 leading-relaxed">
                {post.excerpt}
              </p>
            </div>

            <div className="pt-4 mt-3 border-t border-white/[0.04] flex items-center justify-between text-xs text-slate-400">
